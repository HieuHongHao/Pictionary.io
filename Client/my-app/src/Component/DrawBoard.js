...
const DrawBoard = ({ socket }) => {
  ...
  const handleMouseDown = e => {
    ...
    begin(tool,pos); 
    socket.emit("start-drawing",{tool,pos}); 
  };
  const handleMouseMove = e => {
    ...
    drawing(point,tool);
    socket.emit('incomming-drawing', {point,tool});    
};
  ...
  return (
    <Box borderWidth='10px' borderRadius='20px' borderColor={"teal"} marginLeft={"100px"}>
      <Stage
        width={500}
        height={500}
